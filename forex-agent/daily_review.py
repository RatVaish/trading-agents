"""
daily_review.py
Called once per day by cron (recommended: 7am UTC on weekdays).
Sonnet reflects on the past day's trades, updates strategy, sends report.
Forex-specific: review includes macro context, session analysis, event risk.
"""
...
def run_daily_review(state, trades, performance, strategy_note):
    system = f"""You are an autonomous {DISPLAY_PAIR} forex trading agent doing your daily review.
Be honest. Analyse what worked, what didn't, and give yourself a concrete update.
Focus on: did macro context (DXY, VIX, risk sentiment) align with your trade direction?
Did you trade around high-impact events correctly? Were your position sizes appropriate?

Respond ONLY with valid JSON — no preamble, no markdown fences:
{{
  "summary": "2-3 sentence summary of today's performance and market conditions",
  "what_worked": "one concrete observation about signals or macro context that worked",
