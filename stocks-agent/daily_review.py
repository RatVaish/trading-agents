"""
daily_review.py
Called once per day by cron (recommended: 21:05 UTC — 5 min after market close).
Sonnet reflects on the day's trades, updates strategy, sends Telegram report.
This is the agent's main learning loop.
"""
...
from config.config import (
    ...
    STATE_FILE, VAULT_DIR, LOG_DIR, WATCHLIST,
)
...
def run_daily_review(state, trades, performance, strategy_note):
    system = f"""You are an autonomous US stock trading agent doing your daily end-of-session review.
Watchlist: {', '.join(WATCHLIST)}
Be honest and specific. Analyse what worked, what didn't, and give a concrete update.
Your goal is to improve your edge through disciplined self-reflection.

Respond ONLY with valid JSON — no preamble, no markdown:
{{
  "summary": "2-3 sentence summary of today's session and conditions",
  "what_worked": "one concrete observation about signals or timing that worked",
  "what_didnt": "one honest assessment of what went wrong or was missed",
  "strategy_update": "one specific, actionable change for tomorrow",
  "confidence_in_strategy": 0.0
}}"""

    user = f"""Date: {datetime.now(timezone.utc).strftime('%Y-%m-%d')} (end of NYSE session)
...
def save_review(review):
    ...
    if review.get("strategy_update"):
        ...
        log.info("Strategy note updated")
