...
from config.config import VAULT_DIR, WATCHLIST
...
def compute_performance():
    ...
    try:
        all_trades = get_trades("stocks", limit=10000)
    except Exception:
        return _empty_performance()
    ...
    for sym in WATCHLIST:
        ...
        if sym_trades:
            ...
            per_symbol[sym] = {
                ...
                "win_rate":    round(len(sym_wins) / len(sym_trades), 3),
                "avg_pnl_pct": round(sum(sym_pnls) / len(sym_pnls), 3),
            }
    ...
    if _DB_ENABLED:
        try:
            db_write_performance("stocks", perf)
        except Exception:
            pass
